...
OniStatus Rs2Stream::initialize(class Rs2Device* device, int sensorId, int streamId, std::vector<RIFrameInfo>* profiles)
{
	printf("Rs2Stream.cpp initial Stream\n");
...
OniStatus Rs2Stream::convertDepthToColorCoordinates(StreamBase* colorStream, int depthX, int depthY, OniDepthPixel depthZ, int* pColorX, int* pColorY)
{
	...
	else
	{
        ...
        if (m_needUpdateExtrinsicsDepthToColor) {
			RIFrameInfo* spiDepth = getCurrentProfile();
			RIFrameInfo* spiColor = ((Rs2Stream*)colorStream)->getCurrentProfile();
...
{
	RIFrameInfo* spi = getCurrentProfile();
	//rs2_get_video_stream_intrinsics(spi->profile, &m_intrinsics, nullptr);
	//MV3D_RGBD_CAMERA_INTRINSIC Mv3d_intrinsics;
	//获取相机横纵坐标值
...
RIFrameInfo* Rs2Stream::getCurrentProfile()
{
	SCOPED_PROFILER;

	Mv3dRgbdImageType pixelFormat = convertPixelFormat(m_videoMode.pixelFormat);
	...
	for (auto iter = m_profiles.begin(); iter != m_profiles.end(); ++iter)
	{
		...
		if (sp.streamType == m_rsType
			&& sp.streamType == pixelFormat
			&& sp.nWidth == m_videoMode.resolutionX
			&& sp.nHeight == m_videoMode.resolutionY
			&& sp.framerate == m_videoMode.fps)
		{
			return &sp;
		}
...
bool Rs2Stream::isVideoModeSupported(OniVideoMode* reqMode)
{
	...
	Mv3dRgbdImageType pixelFormat = convertPixelFormat(reqMode->pixelFormat);

	for (auto iter = m_profiles.begin(); iter != m_profiles.end(); ++iter)
	{
		const RIFrameInfo& sp = *iter;

		if (sp.streamType == m_rsType
			&& sp.streamType == pixelFormat
			&& sp.nWidth == reqMode->resolutionX
			&& sp.nHeight == reqMode->resolutionY
